...
The CEPII gravity controls are **archived for transparency only** — they are absorbed by the
fixed effects and drop out for collinearity, so they are not part of the active strategy. See
`99_archived_cepii_gravity_robustness.R`.

> **Committed as a small subset.** The full [CEPII Gravity database](http://www.cepii.fr/CEPII/en/bdd_modele/bdd_modele_item.asp?id=8)
> (`Gravity_V202211.csv`) is ~1.2 GB and is **not** redistributed here. Instead the repo ships
> `data/cepii_gravity_germany_subset.csv` (~36 KB) — exactly the rows the pipeline uses
> (origin Germany → the five origin countries, 2010–2021). This reproduces the archived CEPII
> panels identically without the multi-gigabyte download. To rebuild the subset from the full
> file, filter `Gravity_V202211.csv` to `iso3_o == "DEU"` and the five destination ISO3 codes
> (`AFG`, `ERI`, `IRN`, `IRQ`, `SYR`) for 2010–2021.
...
- **CEPII Gravity database**: Conte, Cosar, Mayer et al.; only a small Germany-origin subset is
  redistributed here (`cepii_gravity_germany_subset.csv`) under CEPII's terms — see link above.
...
The raw data files are UTF-8. Each script sets a UTF-8 `LC_CTYPE` at startup (trying
`en_US.UTF-8`, `C.UTF-8`, `UTF-8`) so the German CSVs read correctly **regardless of the
ambient locale** — including a bare `Rscript` in a `C`/POSIX locale, where they would
otherwise mis-parse. No action needed; this just makes the scripts robust on servers/CI as
well as in interactive RStudio/R.app sessions.
...
│   ├── cepii_gravity_germany_subset.csv    ← tiny CEPII subset (committed)
│   ├── *.rds                  ← intermediate objects (git-ignored; regenerated by the pipeline)
│   └── Gravity_V202211.csv    ← full CEPII, ~1.2 GB (git-ignored; not needed — subset suffices)
└── Chinamere_*.pdf            ← the paper
```
